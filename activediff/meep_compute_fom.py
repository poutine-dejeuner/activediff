...
def mapping(x, eta, beta, Nx, Ny, filter_radius, design_region_width,
    ...
    # up-down symmetry
    # MEEP VERSION COMPATIBILITY FIX (meep >= 1.31.0):
    # mesh_grid now adds +1 to dimensions, but we want to use the actual array dimensions
    # Use x.shape instead of mesh_grid's calculated Nx, Ny
    actual_shape = x.reshape(Nx, Ny).shape
    x = (npa.fliplr(x.reshape(Nx, Ny)) + x.reshape(Nx, Ny))/2
    # filter - pass actual dimensions to avoid mesh_grid recalculation mismatch
    # Use actual array dimensions for Lx, Ly to match what mesh_grid will compute
    actual_Lx = (actual_shape[0] - 1) / design_region_resolution
    actual_Ly = (actual_shape[1] - 1) / design_region_resolution
    filtered_field = mpa.conic_filter(x, filter_radius, actual_Lx, actual_Ly,
