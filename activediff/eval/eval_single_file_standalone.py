...
def mapping(x, eta, beta, Nx, Ny, filter_radius, design_region_width,
    ...
    actual_shape = x.reshape(Nx, Ny).shape
    x = (npa.fliplr(x.reshape(Nx, Ny)) + x.reshape(Nx, Ny)) / 2
    actual_Lx = (actual_shape[0] - 1) / design_region_resolution
    actual_Ly = (actual_shape[1] - 1) / design_region_resolution
    filtered_field = mpa.conic_filter(
        x, filter_radius, actual_Lx, actual_Ly, design_region_resolution)
...
class CompareToTrainClosestImage(EvaluationFunction):
    ...
    def __init__(self, train_set_path: os.PathLike, **kwargs):
        ...
        ts = np.load(train_set_path)
        self.train_set = (ts - ts.min()) / (ts.max() - ts.min() + 1e-8)

    def __call__(self, images: np.ndarray, savepath: str, model_name: str,
                 fom: Optional[np.ndarray] = None, file_format: str = "pdf",
                 **kwargs):
        images = images.squeeze()
...
class VisualizeGeneratedSamples(EvaluationFunction):
    ...
    def __call__(self, images: np.ndarray, savepath: str, model_name: str,
        ...
        save_file = os.path.join(savepath, f"{model_name.lower()}_samples_grid.{file_format}")
        plt.savefig(save_file, dpi=300, bbox_inches='tight', facecolor='white')
        plt.close()
...
class PlotFomHistogram(EvaluationFunction):
    ...
    def __call__(self, images: np.ndarray, savepath: str, model_name: str,
        ...
        save_file = os.path.join(savepath, f"fom_histogram.{file_format}")
...
DEFAULT_TRAIN_SET_PATH = "~/repos/photo/activediff/data/imagesnorm.npy"
...
def evaluate_model(images: np.ndarray, savepath: Path,
    ...
    Args:
        ...
        file_format: File format for saved plots (default: pdf)
    """
    ...
    for eval_cls, kwargs, is_metric in eval_functions:
        ...
        try:
            ...
            out = eval_fn(images=images, fom=fom, savepath=str(savepath),
                          model_name=model_name, file_format=file_format)
            if is_metric:
                results[fn_name] = out
...
def test_compute_fom():
    test_image = np.random.rand(101, 91)
    fom_value = compute_FOM(test_image)
    print(f"FOM for all-zero image: {fom_value}")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file-format", type=str, default="pdf",
                        help="File format for saved plots (default: pdf)")
